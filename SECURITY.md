...
- This project’s **default** ingestion path uses public FTC Events pages (no secrets in CI). Opt-in authenticated FIRST API enrichment (`FIRST_API_USERNAME` / `FIRST_API_TOKEN`) is documented in [docs/first-api.md](docs/first-api.md). Never commit those secrets, never expose them via `VITE_*` / client bundles, and never write them into generated JSON. Production `/ftc-api-proxy` injects Basic auth from Worker/Pages secrets and returns 503 when they are unset.
